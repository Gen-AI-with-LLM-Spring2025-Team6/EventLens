"""
Script to process and load data to a single Snowflake EDW table
"""
import os
import json
import pandas as pd
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Please structure the following event information in a clean, well-organized format. Make sure to highlight important details like event title, date, time, location, and description. Remove any duplicate or irrelevant information:\n\n{text}"
                }
            ],
            temperature=0.3,
            max_tokens=1000
...
def load_to_edw(**context):
    ...
    try:
        ...
        logger.info("Structuring event data using OpenAI...")
        ...
        # Write to Snowflake
        logger.info(f"Loading data to {full_table_name}...")
        result = write_pandas(conn, df, EDW_TABLE)
